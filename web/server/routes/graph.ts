...
import { listMarkdownFilesRecursive } from "../services/markdown-file-listing.js";
...
function buildGraph(wikiRoot: string): GraphData {
  const wikiDir = path.join(wikiRoot, "wiki");
  if (!fs.existsSync(wikiDir)) return { nodes: [], edges: [] };

  const files = listMarkdownFilesRecursive(wikiDir, { skipHidden: true });
...
export function handleGraph(cfg: ServerConfig) {
  return (_req: Request, res: Response) => {
    res.json(buildGraph(cfg.sourceVaultRoot));
  };
}
